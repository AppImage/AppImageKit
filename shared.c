 ...
 * 
 * For found files that are in usr/share/icons, install those icons into the system.
 * This is done within the sqfs_traverse run for performance reaons. */
    ...
    while (sqfs_traverse_next(&trv, &err)) {
        if (!trv.dir_end) {
            ...
            if(r == 0){
                ...
                if(g_str_has_prefix (trv.path, "usr/share/icons/")){
                    gchar *dest;
...
void write_edited_desktop_file(GKeyFile *key_file_structure, char* appimage_path, gchar* desktop_filename, char *md5, gboolean verbose){
    ...
    gchar *filename = g_strdup_printf("applications/appimagekit_%s-%s", md5, desktop_filename);
    gchar *destination = g_build_filename(g_get_user_data_dir(), filename, NULL);
    if(verbose)
...
bool appimage_type2_register_in_system(char *path, gboolean verbose)
{
    ...
    /* Get MIME xml file(s) */
    str_array = squash_get_matching_files(&fs, "(^usr/share/mime/.*.xml$)", md5, verbose); 
    /* Work trough the NULL-terminated array of strings */
    for (int i=0; str_array[i]; ++i) {
        const char *ch = str_array[i]; 
