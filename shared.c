...
void write_edited_desktop_file(GKeyFile *key_file_structure, char* appimage_path, gchar* desktop_filename, gboolean verbose){
    ...
    gchar *tryexec_path = replace_str(appimage_path," ", "\\ "); // FIXME: TryExec does not support blanks
    g_key_file_set_value(key_file_structure, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TRY_EXEC, appimage_path);
    gchar *generated_by = "Generated by appimaged"; // TODO: Path to myself and version
    ...
    gchar *filename = g_strdup_printf(".local/share/applications/appimagekit-%s", desktop_filename);
    gchar *destination = g_build_filename(g_get_home_dir(), filename, NULL);
    if(verbose)
...
bool appimage_type2_register_in_system(char *path, gboolean verbose)
{
    ...
    for (int i=0; str_array[i]; ++i) {
        ...
        if(success){
            ...
            write_edited_desktop_file(key_file_structure, path, desktop_filename, verbose);
        }
