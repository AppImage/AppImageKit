...
class DesktopEntry(IniFile):
    ...
    def checkKey(self, key, value, group):
        ...
        elif re.match("^Name"+Xocale.regex+"$", key):
            pass # locale string

        elif re.match("^GenericName"+Xocale.regex+"$", key):
            pass # locale string

        elif key == "NoDisplay":
            self.checkValue(key, value, type="boolean")

        elif re.match("^Comment"+Xocale.regex+"$", key):
            pass # locale string

        elif re.match("^Icon"+Xocale.regex+"$", key):
            self.checkValue(key, value)
        ...
        elif re.match("^Keywords"+Xocale.regex+"$", key):
            self.checkValue(key, value, list=True)
            self.warnings.append("Key '%s' is a KDE extension" % key)
        ...
        elif re.match("^UnmountIcon"+Xocale.regex+"$", key):
            self.checkValue(key, value)
            self.checkType(key, "FSDevice")
            self.warnings.append("Key '%s' is a KDE extension" % key)
        ...
        elif re.match("^MiniIcon"+Xocale.regex+"$", key):
            self.checkValue(key, value)
            self.warnings.append("Key '%s' is deprecated" % key)
        ...
        elif re.match("^SwallowTitle"+Xocale.regex+"$", key):
            self.warnings.append("Key '%s' is deprecated" % key)
