...
# therefore, by default, on ARM, these tests are not run with FUSE
if [[ "$CI" != "" ]] && [[ "$ARCH" != "arm"* ]] && [[ "$ARCH" != "aarch"* ]]; then
    ...
    echo "Note: this is not a CI environment (or an ARM build), using APPIMAGE_EXTRACT_AND_RUN and patching out magic bytes"
    docker_test_opts+=(
