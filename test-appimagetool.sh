...
"$appimagetool" -l appimagetool.AppImage | grep -q .appimageignore || true
...
if [ "$hash1" != "$hash2" ]; then
    ...
    echo "Hashes for *AppImage.1 and *AppImage2 match"   #Explicit success message in Travis Log is much nicer then silence
fi
