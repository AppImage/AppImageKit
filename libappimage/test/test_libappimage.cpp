...
class AppImageTest : public testing::Test {
 protected:
  ...
  gchar* appImage_type_1_file_path;
  gchar* appImage_type_2_file_path;
  ...
  virtual void SetUp() {
    ...
    path = std::string(TEST_DATA_DIR) + "/AppImageExtract_6-x86_64.AppImage";
    appImage_type_1_file_path = g_strdup(path.c_str());

    path = std::string(TEST_DATA_DIR) + "/Echo-x86_64.AppImage";
    appImage_type_2_file_path = g_strdup(path.c_str());
  }

  virtual void TearDown() {
    g_free(appImage_type_1_file_path);
    g_free(appImage_type_2_file_path);
  ...
  inline void removeTestsDir() { rmdir(tests_dir); }
  ...
  void mk_file(const std::string& path) {
    g_file_set_contents(path.c_str(), test_file_content.c_str(),
                        test_file_content.size(), 0);
  }

  void rm_file(const std::string& path) { g_remove(path.c_str()); }
  ...
  bool areIntegrationFilesDeployed(const std::string& path) {
    ...
    char* apps_path =
        g_strconcat(g_get_user_data_dir(), "/applications/", NULL);
...
TEST_F(AppImageTest, check_appimage_type_1) {
  int t = check_appimage_type(appImage_type_1_file_path, 0);
  ASSERT_EQ(1, t);
}

TEST_F(AppImageTest, check_appimage_type_2) {
  int t = check_appimage_type(appImage_type_2_file_path, 0);
  ASSERT_EQ(2, t);
}

TEST_F(AppImageTest, appimage_register_in_system_with_type1) {
  int r = appimage_register_in_system(appImage_type_1_file_path, true);
  ASSERT_EQ(0, r);

  ASSERT_TRUE(areIntegrationFilesDeployed(appImage_type_1_file_path));

  appimage_unregister_in_system(appImage_type_1_file_path, false);
}

TEST_F(AppImageTest, appimage_register_in_system_with_type2) {
  int r = appimage_register_in_system(appImage_type_2_file_path, true);
  ASSERT_EQ(0, r);

  ASSERT_TRUE(areIntegrationFilesDeployed(appImage_type_2_file_path));

  appimage_unregister_in_system(appImage_type_2_file_path, false);
}

TEST_F(AppImageTest, appimage_type1_register_in_system) {
  bool r = appimage_type1_register_in_system(appImage_type_1_file_path, false);
  ASSERT_EQ(true, r);
...
TEST_F(AppImageTest, appimage_type2_register_in_system) {
  bool r = appimage_type2_register_in_system(appImage_type_2_file_path, false);
  ASSERT_EQ(true, r);
